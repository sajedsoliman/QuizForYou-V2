...
// Material-UI imports 
import { Card, Collapse, makeStyles, Typography, useTheme } from '@material-ui/core'
...
export const Question = React.memo(({ question, increaseCorrectAnswers, purpose = "public" }) => {
    ...
    return (
        <Flippy isFlipped={flipped}
            flipOnClick={false}
            style={{ border: darkMode && "1px solid rgb(255 255 255 / 13%)", borderRadius: 10, /* ...(flipped && result == "Right" ? { transform: "translate3d(0,0,15px)" } : null) */ }}>
            <FrontSide>
                {/* Question title */}
                ...
                <Collapse in={collapsed}>
                    {
                        purpose == "public" ? (
                            ...
                            <Typography>Correct Answer: <Typography display="inline" color="error">{correctAnswer}</Typography></Typography>
                        )
                    }
                ...
                <div className={classes.collapseIcon}>
                    <IF condition={collapsed}
                        elseChildren={<ArrowDownward color={darkMode ? "secondary" : "inherit"} onClick={handleCollapseAnswers} />}>
                        <ArrowUpward color={darkMode ? "secondary" : "inherit"} onClick={handleCollapseAnswers} />
                    </IF>
                </div>
